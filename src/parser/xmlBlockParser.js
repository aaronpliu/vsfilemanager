        ...
        for (const deletedKey in deletions) {
            ...
            for (let i = parts.length - 1; i > 0; i--) {
                ...
                if (targetBlocks.hasOwnProperty(parentKey) && targetBlocks[parentKey].originalType === 'object') {
                    // Found a parent object block, modify its XML content to remove the deleted element
                    const parentBlock = targetBlocks[parentKey];
                    const elementName = parts[i]; // The element to remove
                    
                    // Parse the parent's XML content
                    const options = {
                        ignoreAttributes: false,
                        attributeNamePrefix: '@_',
                        textNodeName: '#text',
                    ...
                    try {
                        const parentXmlObj = parser.parse(parentBlock.value);
                        
                        // Remove the element from the parsed object
                        if (parentXmlObj.hasOwnProperty(elementName)) {
                            delete parentXmlObj[elementName];
                            
                            // Convert back to XML and update the parent block
                            const builder = new XMLBuilder(options);
                            parentBlock.value = builder.build(parentXmlObj);
                        }
                    } catch (e) {
                        // If we can't parse and modify the XML, leave it as is
                    }
